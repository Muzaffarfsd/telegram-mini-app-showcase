...
### World-Class Chat UX Improvements (April 2026 — Phase 3)
1. **Typewriter Effect** — `useTypewriter` hook in AIAgentMessage with RAF-based char-by-char animation. Reveals 35% of remaining characters per frame for natural variable-speed typing. Handles SSE `replace` events (shorter content) by resetting to empty and re-animating. Instantly completes when `isStreaming=false`.
2. **Thinking Indicator with Timer** — `ThinkingIndicator` component shows 3 bouncing dots + elapsed seconds counter before first token arrives. `setInterval` in AIAgentPanel tracks thinking duration. Bouncing animation via `ai-thinking-bounce` keyframe. Persona-colored dots.
3. **Smart Suggested Replies** — Per-deal-stage presets (awareness→action) shown after each AI response. 3 contextual quick-reply buttons that match the current funnel stage. Localized (ru/en). Appear below last assistant message when AI finishes responding.
4. **Scroll-to-Bottom Pill** — Floating "↓" button appears when user scrolls up (threshold: `scrollHeight - scrollTop - clientHeight < 60`). Animated entrance/exit via framer-motion. Smooth scroll on tap. Glass morphism styling. Auto-scrolling pauses when user scrolls up during streaming.
5. **Long-Press Context Menu** — 500ms pointer-down timer triggers iOS-style context menu. Also available via right-click/contextmenu. Actions: Copy, Reply, Speak (for assistant messages). Glass background with blur, `ai-menu-in` scale animation. Click-outside dismissal. Unmount cleanup prevents leaked timers.
6. **Conversation Insights Panel** — Toggle via chart icon in header (visible after 2+ messages). Shows: multi-color stage progress bar (5 segments, each stage has unique color gradient), message count, readiness percentage, current stage label. Localized stage labels (ru/en). Animated open/close.
7. **Voice Waveform Visualization** — Live canvas-rendered frequency bars during recording/speech recognition. Uses Web Audio API AnalyserNode (fftSize=128, smoothing=0.7). 24 rounded bars with amplitude-mapped height and opacity. Recording timer shows elapsed time (MM:SS). Red recording indicator dot with pulse animation. Proper cleanup of AudioContext, MediaStream tracks, and animation frames.

### Chat UX Parity with ChatGPT/Claude/iMessage (April 2026 — Phase 4)
8. **Stop Generation Button** — Red square stop button replaces send arrow during streaming. Wired to `stopGeneration()` which aborts SSE fetch, finalizes the last streaming message, resets `isLoading`. Located in `AIAgentInput`.
9. **Image Preview in Chat Bubbles** — Uploaded images now render as thumbnails (max 200×150px, rounded 12px) inside user message bubbles. Full base64 data URL stored in-memory only (stripped from localStorage to prevent quota overflow).
10. **Message Entrance Animations** — Each message wrapped in framer-motion `m.div` with `initial={{ opacity: 0, y: 12, scale: 0.97 }}` → `animate={{ opacity: 1, y: 0, scale: 1 }}`. 250ms ease-out transition. iMessage-like slide-up appearance.
11. **Fixed Numbered Lists** — Regex changed from `$&` (captures entire match) to `$1.` + `$2` (captures number and text separately). Proper `<span>` for number and `<span>` for text content.
12. **Clickable Markdown Links** — `[text](url)` converted to `<a>` tags with emerald underline style. Protocol-sanitized (only http/https allowed, others replaced with `#`). Bare URL auto-detection for `https?://` patterns with context-aware skip for already-wrapped links.
13. **Code Block Copy Button** — Positioned absolute top-right in `<pre>` blocks. `onclick` copies `previousElementSibling.textContent`. Shows "✓" feedback for 1.5s. Glass-morphism styling matching code block theme.
14. **Error Retry Button** — Red "Повторить" button with retry icon appears on error messages. Uses `message.isError` flag (set on both SSE error and network catch). Contextual retry: finds the user message immediately before the error and re-sends it, removing both the error response and original user message. Passes `errorMessageId` to ensure correct message pair is retried.
15. **Persona Emoji Avatars** — Replaced single-letter avatars with expressive emojis: Алекс 🧑‍💼, Марина 🎨, Артём 💻, Ольга 📊. Applied in both panel header and in-message persona badges via `PERSONA_EMOJIS` mapping.

### Phase 5: AI Beyond Chat Panel (April 2026)
- **Quick Action Menu** — Long-press (400ms) on AI FAB reveals page-contextual action menu (3 actions + open chat). Actions defined per route component (showcase/projects/demoApp/demoLanding/constructor) with ru/en labels. Glass-morphism backdrop, staggered animation.
- **Mini Chat Preview Bubble** — Shows last AI message excerpt (80 chars) near FAB 5 seconds after page load. Auto-hides after 6s. Only visible when chat panel is closed.
- **AI Toast Notifications** — Top-screen glass overlay for AI notifications. Triggered via `CustomEvent("ai-toast")` from action blocks or shake handler. Timer-safe with ref-based cleanup preventing race conditions.
- **Shake-to-Ask** — DeviceMotion API with speed threshold 20, cooldown 3s. On shake: haptic feedback, toast with page context, auto-opens chat. Uses `handleOpenRef` to avoid stale closures.
- **Spotlight/Highlight Action** — AI can highlight page elements via `spotlight` action block with CSS selector. Selector validation: only simple selectors allowed (data-attributes, IDs, single classes). Duration capped 1-10s. Element receives `ai-spotlight-highlight` CSS class with pulsing green border and scroll-into-view.
- **Toast Action** — AI can show toast notifications from action blocks even when chat is closed. `{"type":"toast","message":"..."}` dispatches CustomEvent.
- **Proactive Messages** — Context-aware proactive messages appear after 15s on specific pages (showcase/projects/demoApp/constructor). Auto-dismiss after 8s. Dismissed pages tracked in ref Set.
- **Timer Lifecycle Hardening** — All auto-hide timers (toast, preview, proactive, shake-toast) use refs with cancellation before setting new timers and in effect cleanup, preventing race conditions from rapid events or component unmount.

### Phase 6: World-Class AI Interactions (April 2026)
- **Exit Intent Detection** — `visibilitychange` listener detects tab/app switching. Page-contextual farewell messages with personalized offers. 30s cooldown between triggers. Messages in `EXIT_MESSAGES` map per route component.
- **Idle Re-engagement** — 60s inactivity timer resets on any user interaction (pointer/key/scroll/touch). Shows contextual AI suggestion toast. `IDLE_MESSAGES` map per route with ru/en variants.
- **Smart Scroll Triggers** — RAF-throttled scroll listener with `scrollTriggeredRef` Set for deduplication. Detects: pricing section visibility (IntersectionObserver-style rect check), features section, 70%+ scroll depth. Each triggers context-aware AI toast once per page.
- **Smart Clipboard Detection** — `copy` event listener analyzes selected text. Detects prices (₽/руб/$) vs long text vs short text. Shows clipboard suggestion bubble near FAB with dismiss button. 6s auto-hide with timer ref.
- **Guided Tour Overlay** (`AIGuidedTour.tsx`) — SVG mask with cutout around target element. `SAFE_SELECTOR` validation. Auto-positioning tooltip (top/bottom). Step-by-step navigation with progress bar. Keyboard accessible (Escape/Enter/Arrow keys). `role="dialog"` + `aria-modal`. Animated cutout border glow. AI triggers via `guided_tour` action block.
- **AI Session Summary** (`AISessionSummary.tsx`) — Stats grid (pages/demos/time). Visited pages as tags. AI recommendation based on behavior. Glass-morphism card. Keyboard Escape dismiss. `role="dialog"` + `aria-modal`. AI triggers via `session_summary` action block.
- **Swipe Gestures on FAB** — Touch gesture detection via `useSwipeGesture` hook. Swipe up = open chat. Swipe left/right = cycle AI persona (dispatches `ai-persona-cycle` CustomEvent). 50px threshold, 500ms max duration.
- **Voice Navigation** — `parseVoiceNavigation` maps voice commands to routes (демо ресторана → /demos/restaurant/app, проекты → /projects, etc.). Integrated in sendMessage — voice mode intercepts nav commands locally without API call.
- **Persona Cycling** — `ai-persona-cycle` CustomEvent listener in useAIAgent. Cycles through 4 personas (Алекс → Марина → Артём → Ольга). Triggered by FAB swipe gestures.
- **New Files**: `client/src/hooks/useAIInteractions.ts` (interactions hook + swipe gesture hook + voice nav parser + session summary util), `client/src/components/AIAgent/AIGuidedTour.tsx`, `client/src/components/AIAgent/AISessionSummary.tsx`
- **System Prompt**: Added `guided_tour` and `session_summary` action type instructions
- **Unified Timer Cleanup** — Master cleanup effect in AIAgentButton clears all 6 timer refs on unmount
- **Accessibility** — Dialog roles, aria-modal, keyboard navigation (Escape/Enter/Arrow), aria-labels on icon-only buttons for tour and summary components

### Phase 7: Final World-Class AI Features (April 2026)
- **AI Memory (Cross-Session Persistence)** — `localStorage` key `web4tg_ai_memory` stores userName, businessType, businessName, preferences, lastTopics, interactionCount, firstSeen/lastSeen. `extractMemoryFromText()` regex extracts name (меня зовут/my name is), business type (9 categories: ресторан, салон, магазин, фитнес, клиника, отель, авто, образование, недвижимость), business name (компания/бренд), and conversation topics. Memory sent to server via `memory` field in `/api/ai/chat` payload. Server validates with strict type guards (`validateMemory()`) and injects into Gemini system prompt via `[ПАМЯТЬ О КЛИЕНТЕ]` section. AI uses name naturally, references previous topics, and greets returning users warmly.
- **Contextual Thinking Indicator** — `getThinkingPhrase(message, lang)` maps user message content to 16 contextual Russian/English phrases: "Считаю стоимость..." for price queries, "Подбираю решение для ресторана..." for niche queries, "Анализирую рынок..." for competitor questions, etc. Shown in both the message bubble (`ThinkingIndicator` component with italic styling) and the panel header during AI response generation. Seconds counter only appears after 2s.
- **Thumbs Up/Down Feedback** — Replaced single HeartIcon with dual `ThumbsUpIcon`/`ThumbsDownIcon` SVG icons. Feedback state persisted per message (`message.feedback` field) via `setMessageFeedback()` in useAIAgent. State survives panel close/reopen. Visual scale animation (1.15x) on selection. "Спасибо за отзыв! 👍" / "Учтём, спасибо 🙏" toast with fade-in animation. Analytics event `message_feedback` posted to `/api/analytics/events`.
- **Dynamic Follow-up Chips** — Enhanced `SUGGESTED_REPLIES_BY_STAGE` with `dynamicSuggestions` that analyze last AI response content via regex: detects price/demo/timeline/MVP/ROI/payment keywords and generates contextual follow-up suggestions. Sparkle emoji (✨) prefix on each chip. Animated entrance with staggered `m.button` (0.06s delay per chip). Falls back to stage-based defaults when no contextual match.
- **Smart Comparison Chips** — Tracks visited demo pages via `pagesVisited.filter("/demos/*/app")`. When 3+ demo pages visited, shows orange comparison chip "⚖️ Сравнить: Ресторан, Салон, Магазин" with animated entrance (0.35s delay). Click sends structured comparison request to AI. Demo name mapping for 9 demo types with ru/en localization.
