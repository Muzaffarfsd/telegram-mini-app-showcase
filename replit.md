...
- **ShowcasePage Cinematic Redesign** (v2 — 15 audit improvements): Syne 800 + Instrument Serif + Inter. OLED #050505 + SVG noise overlay (fractalNoise 0.04 opacity, mixBlendMode: overlay). Sections: Hero (lazy-loaded video bg, rotating words in Syne with gradient, "22 приложения запущено" eyebrow with live dot), Marquee, Social Proof Strip (22 apps / 5 industries / since 2024), Big Numbers → Card Stack (reordered for persuasion logic), Lucide-react icon Feature Grid (replaced emoji), Process (01-03), Dual Testimonials (+340% numeric card + editorial blockquote), CTA, Footer. **Card Stack**: swipe gestures (touch-pan-y), auto-advance every 4.5s with Play/Pause button, dot pagination (aria-current), spring animation. **Accessibility**: useReducedMotion hook (single matchMedia listener, passed as prop to Cin/Ct), ALL hero animations gated for reduced-motion, WCAG AA contrast ≥0.5 alpha on all text, focus-visible on all interactives, semantic HTML5, no tab-role misuse. **Performance**: lazy video (preload=none + IntersectionObserver), proper timeout cleanup via useRef. LazyMotion strict: only m.* from @/utils/LazyMotionProvider.
