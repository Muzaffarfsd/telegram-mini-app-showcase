...
12. **Telegram WebApp Bot API 8.0 Prefetch (✅ ADDED)**: Created `client/src/lib/telegramPrefetch.ts` with intelligent prefetching. Features: Bot API 8.0 version detection (activated/deactivated events), viewport-aware prefetch triggers, network-aware loading (4G detection), requestIdleCallback optimization. Hooks: `useDemoPrefetchOnHover`, `useDemoPrefetchOnVisible`, `useImagesPrefetch`, `useNetworkAwarePrefetch`.
