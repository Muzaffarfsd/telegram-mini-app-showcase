...
import { ArrowRight, Menu, X, Sparkles, MessageCircle, FileText, Settings } from "lucide-react";
...
export default function ProjectsPage({ onNavigate, onOpenDemo }: ProjectsPageProps) {
  ...
  return (
    ...
    >
      ...
      {/* SIDEBAR PANEL */}
      <div 
        className={`fixed top-0 left-0 h-full z-50 transition-transform duration-300 ease-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{ 
          width: '280px',
          background: '#0A0A0B',
      ...
      >
        ...
        <nav style={{ padding: '16px 12px' }}>
          ...
          {/* Nav Items */}
          {[
            { icon: Sparkles, label: 'Демо приложения', section: 'projects', active: true },
            { icon: MessageCircle, label: 'Консультация', section: 'ai' },
            { icon: FileText, label: 'О нас', section: 'about' },
            ...
            >
              <item.icon size={18} color={item.active ? '#FAFAFA' : '#71717A'} />
              <span style={{
                fontSize: '14px',
                fontWeight: 500,
                color: item.active ? '#FAFAFA' : '#A1A1AA'
              }}>
                {item.label}
              </span>
            </button>
