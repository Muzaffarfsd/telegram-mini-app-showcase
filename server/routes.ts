...
<body>
  ...
  // Get all active and approved user stories
  ...
  app.get("/api/user-stories/my", verifyTelegramUser, async (req: any, res) => {
    try {
  ...
  // Get upload URL for story media
  app.post("/api/user-stories/upload-url", verifyTelegramUser, async (req: any, res) => {
    try {
      const objectStorageService = new ObjectStorageService();
      const uploadURL = await objectStorageService.getObjectEntityUploadURL();
      res.json({ uploadURL });
  ...
  app.post("/api/user-stories", verifyTelegramUser, async (req: any, res) => {
    try {
      const telegramId = req.telegramUser.id;
      ...
      // Validate that URL is from our object storage
  ...
  app.patch("/api/user-stories/:id", verifyTelegramUser, async (req: any, res) => {
    try {
  ...
  app.post("/api/user-stories/:id/view", async (req, res) => {
    try {
      const storyId = parseInt(req.params.id);
  ...
  // Soft-delete user's own story (deactivate)
  app.delete("/api/user-stories/:id", verifyTelegramUser, async (req: any, res) => {
    try {
      const storyId = parseInt(req.params.id);
      const telegramId = req.telegramUser.id;
